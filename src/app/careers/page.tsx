    ...
    <div>
      ...
      <div className="container mx-auto w-full">
        ...
        <div className="flex h-auto min-h-screen">
          <div className="w-[314px] h-full ">
            <CategoryMenu categories={categories} setCategory={setCategory} />
          </div>
          <div>
            {job?.length ? (
              job.map((jobItem) => (
                <div key={jobItem.id} className="border p-4 my-2 rounded-md">
                  <h3 className="font-semibold">{jobItem.label}</h3>
                  <p className="text-gray-600">{jobItem.category}</p>
                </div>
              ))
            ) : (
              <p>No jobs found for selected categories.</p>
            )}
          </div>
        </div>
        {/* Pagination Controls */}
